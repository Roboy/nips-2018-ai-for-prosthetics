...
class Critic:
    ...
    def __init__(self, sess: tf.Session, state_dim: int, action_dim: int,
        ...
        self.online_nn = TFQNetwork(sess=sess, state_dim=state_dim, action_dim=action_dim)
        ...
        self.target_nn = TFQNetwork(sess=sess, state_dim=state_dim, action_dim=action_dim)
    ...
    def _setup_target_nn_update_ops(self, tau):
        update_ops = []
        for running_var, target_var in zip(self.online_nn._variables, self.target_nn._variables):
    ...
    def online_nn_train(self, states_batch, actions_batch, q_values_batch) -> None:
        self.sess.run([self._online_nn_train_op], feed_dict={
